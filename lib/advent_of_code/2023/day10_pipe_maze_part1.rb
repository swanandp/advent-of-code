...
def neighbours(maze, x, y)
  neighbour_offsets[maze.dig(x, y)].map do |(i, j)|
    if 0 <= x + i && x + i < maze.length && 0 <= y + j && y + j < maze.first.length
      [x + i, y + j]
    end
  end.compact
end
...
def loop_distance(maze, start, pipe)
  x, y = start
  loop_length = 0
  visited = maze.length.times.map { |i| maze.first.length.times.map { |j| -1 } }
  maze[x][y] = pipe
  visited[x][y] = loop_length
  l, r = neighbours(maze, x, y)
  loop_length += 1
  ...
  while l != r
    ...
    visited[lx][ly] = loop_length
    visited[rx][ry] = loop_length

    l = neighbours(maze, lx, ly).select { |(i, j)| visited[i][j] == -1 }.first
    r = neighbours(maze, rx, ry).select { |(i, j)| visited[i][j] == -1 }.first
    loop_length += 1
  end
