...
game_id_scanner = "Game %d:"
red_matcher = /((\d+)\sred)/
green_matcher = /((\d+)\sgreen)/
blue_matcher = /((\d+)\sblue)/
red_limit = 12
green_limit = 13
blue_limit = 14
sum = 0
...
DATA.each_line do |line|
  ...
  game_id, _ = head.scanf(game_id_scanner)
  game_possible = true

  tail.split(";").each do |draw|
    red_count = draw.match(red_matcher)&.send(:[], 2).to_i
    green_count = draw.match(green_matcher)&.send(:[], 2).to_i
    blue_count = draw.match(blue_matcher)&.send(:[], 2).to_i
    ...
    # p [game_id, red_count, green_count, blue_count]
  end

  if game_possible
    sum += game_id
  else
    # puts "Game #{game_id} is impossible"
  end
end
