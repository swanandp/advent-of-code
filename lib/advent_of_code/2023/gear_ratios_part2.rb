...
def sum_of_parts(parts)
  ...
  parts.each_with_index do |part, i|
    ...
    part.chars.each_with_index do |c, j|
      ...
      if number_stack.length > 0 && (!(c =~ /\d/) || j == part.length - 1)
        ...
        star_neighbours = neighbours(
          i,
          indexes_stack.first,
          indexes_stack.last,
          parts.length,
          part.length
        ).select { |r1, c1| parts[r1][c1] == "*" }

        star_neighbours.each do |star_neighbour|
          stars[star_neighbour] ||= []
          stars[star_neighbour] << number
        end
  ...
  stars
    .select { |star, numbers| numbers.length == 2 }
    .reduce(0) { |memo, (star, numbers)| memo += (numbers[0] * numbers[1]) }
end

# p sum_of_parts(sample_lines)
