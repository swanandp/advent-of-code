...
def extract_number(line)
  word = line.strip
  first_digit = extract_first_digit(word)
  last_digit = extract_last_digit(word)
  digit_to_number(first_digit) * 10 + digit_to_number(last_digit)
...
def valid_first?(str)
  @regex_f ||= /(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine)|(1)|(2)|(3)|(4)|(5)|(6)|(7)|(8)|(9)/
  str.match(@regex_f)&.send(:[], 0)
end

def valid_last?(str)
  @regex_l ||= /(eno)|(owt)|(eerht)|(ruof)|(evif)|(xis)|(neves)|(thgie)|(enin)|(1)|(2)|(3)|(4)|(5)|(6)|(7)|(8)|(9)/
  str.match(@regex_l)&.send(:[], 0)
end
...
def extract_last_digit(word)
  current = ""

  word.reverse.chars.each do |char|
    current += char

    if valid_last?(current)
      break
    end
  end
